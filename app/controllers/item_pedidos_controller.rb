class ItemPedidosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @item_pedido.save
        format.html { redirect_to(@item_pedido, :notice => 'ItemPedido was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @item_pedido.update_attributes(params[:item_pedido])
        format.html { redirect_to(@item_pedido, :notice => 'ItemPedido was successfully updated.') }
        format.xml  { head :ok }
      else
